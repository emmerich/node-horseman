...
var DEFAULTS = {
	...
	phantomServerAddress: undefined,
	jQueryPath: path.join(__dirname, '../files/jquery-2.1.1.min.js')
};
...
var Horseman = function Horseman(options) {
	...
	if (typeof this.options.bridgePath !== 'undefined') {
		instantiationOptions['bridgePath'] = this.options.bridgePath;
	}
	if (typeof this.options.findstrLocation !== 'undefined') {
		instantiationOptions['findstrLocation'] = this.options.findstrLocation;
	}
	if (typeof this.options.useSystemFindstr !== 'undefined') {
		instantiationOptions['useSystemFindstr'] = this.options.useSystemFindstr;
	}
	if (typeof this.options.findPortBuffer !== 'undefined') {
		instantiationOptions['findPortBuffer'] = this.options.findPortBuffer;
	}
	if (typeof this.options.phantomServerAddress !== 'undefined') {
		instantiationOptions['phantomServerAddress'] = this.options.phantomServerAddress;
	}
		...
		function loadFinishedSetup(status) {
			...
			.then(function injectJQuery() {
				...
				.then(function(hasJquery) {
					...
					return HorsemanPromise.fromCallback(function(done) {
						return page.injectJs(self.options.jQueryPath, done);
					})
					.tap(function(successful) {
						if (!successful) {
							var err = new Error('jQuery injection failed at path ' + self.options.jQueryPath);
							return HorsemanPromise.reject(err);
						}
						debug('injected jQuery from ' + self.options.jQueryPath);
					});
				});
			})
